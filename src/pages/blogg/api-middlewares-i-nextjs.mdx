...
summary: Så här kan man skapa återanvändbara middlewares för API-endpoints i Next.js
date: 2020-07-09
---
...
Kortfattat innebär det att om man skapar en fil i sitt Next.js projekt under **pages/api/users.js** ungefär såhär:
...
**Men hur gör man då med middlewares om man har API routes som är strukterade på det här sättet?** En del routes i Next.js applikationer kanske man vill skydda bakom nån slags authenticering med en API nyckel exempelvis. Hur gör man då?
...
**Men hur ska man då egentligen göra med Next.js och våran default exported funktion?**
...
Man skapar först en fil, **withAuthorization.js** i katalogen `middlewares/`:
...
Om nyckeln saknas eller inte är valid så kommer curl-anropet returnera en **403** med `{ message: 'This route requiers authorization' }`.
