...
Har man flera komponenter med likande media queries med just **700px** hårdkokad i varje komponent måste varje komponent hållas i synk om man ändrar sin breakpoint-skala. Om man vill ändra 700px till 675px exempelvis.
...
Ovanstående exempel innebär att komponenter själva _inte_ måste hålla reda på breakpoint-skalan innehåller för faktiska värden.
...
### Dock finns det ju såklart i alla fall en nackdel värd att ta upp med den här lösningen
...
```js
const getBreakpointScaleFromTheme  = (breakpoint) => css(({ theme }) => theme.breakpoints[breakpoint])
...
Oj...Nu vart det kanske lite mycket på en gång 🧐

Den minsta funktionen `getBreakpointScaleFromTheme` tar som parameter en breakpoint, som i det här fallet är **small**, **medium**, eller **large**.

Den funktionen använder i sina tur `css()`. Vad är då det för något? Det är en hjälpfunktion som exponerar hela ThemeContext, men även eventuella props på en styled component.

`getBreakpointScaleFromTheme` ser alltså till att läsa av ThemeContext och hämta ut efterfrågad breakpoint-värde.

`breakpointToMediaQuery` är den funktion som bygger upp själva media queryn. Även denna funktion tar en breakpoint som parameter.
...
Slutligen så exporteras ett objekt som innehåller olika variationer av den **templateStrings-funktion** som `breakpointToMediaQuery` returnerar.
...
- **media.small** mappas mot det pixelvärde man har specificerat i Context på theme.breakpoints.small
- **media.medium** mappas mot det pixelvärde man har specificerat i Context på theme.breakpoints.medium
- **media.large** mappas mot det pixelvärde man har specificerat i Context på theme.breakpoints.large
...
- Breakpoint-skalan har till **stor del abstraherats bort från komponenterna** i och med Context av avläsning av Context i hjälpfunktionerna.
