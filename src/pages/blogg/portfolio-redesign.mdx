...
> Under utveckling använde jag `@media (prefers-color-mode: dark) { ... }` istället för att styra ljust/mörkt tema. Men i slutändan valde jag att låta användaren styra själv över det valet.   
Att byta implementation (till en React hook fipplar med css class på body-elementet) var väldigt enkelt då jag bara behövde ändra hur `setupThemeVariables()` anropades.
...
Till och med **marginaler är komponenter**. Jag har arbetat en del i ett Reactbaserade design system där marginaler användes som reactkomponenter och har på ett sätt fattat tycke för det, då man väldigt snabbt kan få en konsekvent layout på komponenter.
...
<ResponsiveGrid itemWidth="200px" gutter="10px" style={{marginBottom: '10px'}}>
