...
> Sammanfattningen går inte in i detalj hur styled-components och styled-icons fungerar. Sammanfattningen använder mer de två för att visa ett exempel på hur man kan strukturera ui-komponenter med React och några saker att tänka på.
...
I ett första stadie såg skapade jag ikoner på följande sätt:
...
> `${!size && defaultSize}` används för att styled-icons har en prop, size, som sätter en storlek på ikonerna. Om ingen storlek sätts så faller basikonen till en defaultstorlek.
