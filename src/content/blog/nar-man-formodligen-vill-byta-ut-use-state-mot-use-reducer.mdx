...
summary: Reflektioner om när useReducer är ett bättre val än useState.
date: 2021-02-13
...
## Fas 1: useState-variabler överallt
...
Vilka fördelar kunde vi då se när vi refaktorerade från useState till useReducer?

1. **Fokus på "vad" istället för "hur":** Formulär-komponenten behövde inte längre veta exakt hur state skulle uppdateras.
2. **Automatisk hantering av beroenden:** Reducern skötte beroenden mellan sub-värden (som att rensa feedback).
3. **Enklare att följa flödet:** All state-logik samlades på ett ställe istället för utspridd i callbacks.

Den allra största fördelen var den första punkten. Om man ska ta exemplet med att låta användaren backa i formuläret så gick vi från:
...
Vår formulär-reducer såg då till att själv antingen bara backa ett steg eller rensa hela formuläret, beroende på vilket steg användaren navigerade till.
