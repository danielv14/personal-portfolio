...
Den är väldigt enkel och straight-forward, men har några nackdelar:

- **Komponenten äger sin egen breakpoint:** Varje komponent måste själv hålla reda på vilken bredd som gäller för responsiva ändringar.
- **Mobile-first är inte explicit:** Varje komponent måste veta att `min-width` används (och inte `max-width`).
- **Svårt att hålla synkade:** Har man flera komponenter med `min-width: 700px` hårdkodat måste alla uppdateras om breakpoint-skalan ändras.
- **Pixelvärden är inte beskrivande:** 700px säger ingenting om vad det representerar - exempelvis "tablets och uppåt".
...
- Varje responsiv komponent måste importera breakpoint-skalan.
- Komponenterna måste fortfarande veta att `min-width` gäller (inte `max-width`).
- Komponenterna behöver alltså fortfarande känna till implementationsdetaljer kring hur media queries ska skrivas.
