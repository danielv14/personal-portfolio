...
summary: Mobile-first design, CSS-variabler och styled-components.
date: 2020-06-30
...
- **CSS variabler** - Gör snabbt ändringar som slår över hela sajten genom att ändra några variabler.
...
```typescript title="src/theme/colors.ts" {1,2,3,4,5,6,7,8,9} showLineNumbers
...
```typescript title="src/theme/setupThemeVariables.ts" showLineNumbers
...
```typescript title="src/components/ui/globalStyle.tsx" showLineNumbers
...
```typescript title="src/components/ui/margins/marginLarge.tsx" showLineNumbers
...
```typescript title="src/components/cardPost.tsx" showLineNumbers
