...
```javascript title="nuxt.config.json" showLineNumbers
// Other configuration
plugins: [
'@/plugins/api',
],
...
```javascript title="plugins/api.js" {5,6} showLineNumbers
...
```javascript title="API.js" showLineNumbers
class API {
	constructor({ request }) {
...
```javascript title="example-vue-component.vue" {4,5,6} showLineNumbers
